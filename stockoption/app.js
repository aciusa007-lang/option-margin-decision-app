...
async function fetchCboeVixSeries(startDate){ const response = await fetch("/stockoption/data/vix-history.csv?v=20260320"); if (!response.ok) throw new Error(`HTTP ${response.status}`); const csv = await response.text(); const series = parseCboeCsv(csv, startDate); if (!series.length) throw new Error("Unable to load VIX history."); return series; }
